...
| Kombination  | Beide zusammen                        | • Wenn `exam_date` ≤ heute **und** `updated_at` ≥ `exam_date` → 100 %<br>• Wenn `updated_at` < `exam_date` → 0 %, da Untersuchung geplant, aber veraltet.<br>• Wenn `updated_at` alt, obwohl `exam_date` in Zukunft → Hinweis. |
